...
function initializeLearningPage(user, userData) {
    ...
    get(courseRef).then(snapshot => {
        ...
        sectionsAreaEl.innerHTML = `
            <h2>Welcome, ${userData.displayName}!</h2>
            <p>This is your learning dashboard for the ${courseData.title}.</p>
            <p>${courseData.description}</p>
        `;

        const userProgressSnapshot = await get(ref(db, `users/${user.uid}/progress/${courseId}`));
        ...
        modules.forEach(module => {
            ...
            moduleWrapper.classList.add('course-section');
            ...
            if (sections.length === 0) {
                ...
                moduleWrapper.appendChild(noContent);
            } else {
                sections.forEach(section => {
                    ...
                    if (contentItems.length === 0) {
                        ...
                        contentItems.forEach(item => {
                            ...
                            let contentHtml = `<h5>${item.title}</h5>`;
                            switch (item.type) {
                                case 'module':
                    ...
                    moduleWrapper.appendChild(sectionWrapper);
                });
            }
            sectionsAreaEl.appendChild(moduleWrapper);
        });
    ...
    try {
        ...
        if (courses) {
            ...
            for (const courseId in courses) {
                const course = { ...courses[courseId], courseId };
...
async function loadEnrolledCourses(userId, enrolledCoursesData, userProgress) {
    ...
    for (const enrollment of enrolledCoursesData) {
        ...
        try {
            ...
            console.error(`Error loading enrolled course ${courseId}:`, error);
        }
    }
}
